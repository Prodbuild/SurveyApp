    ...
    public class BBMessage
    {
        ...
        private void KeepAlive(object state)
        {
            lock (_lastMessages)
            {
                ...
                while (myEnum.MoveNext())
                {
                    sendStatusMessage((string)myEnum.Key, ((BBInfo)myEnum.Value)._status, ((BBInfo)myEnum.Value)._msg, false);
                }
            }
        }
        ...
        private static log4net.ILog log = log4net.LogManager.GetLogger(typeof(BBMessage));
    }
